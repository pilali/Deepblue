...
/* One value per control port, copied from LV2 ports / JUCE parameters.
   ...
   field ORDER must match the LV2 control-port indices 4..15 (the wrapper builds
   this struct positionally from ctl(P_DEPTH)..ctl(P_REVERB_SIZE)). */
typedef struct {
    float depth;        /* idx 4   [0 – 1]   macro: surface → deep water     */
    float tone;         /* idx 5   [0 – 1]   bright/dark trim around depth    */
    float wobble;       /* idx 6   [0 – 1]   pitch-wavering depth             */
    float wobble_rate;  /* idx 7   [0.05 – 2] Hz   wavering speed             */
    float dispersion;   /* idx 8   [0 – 1]   all-pass frequency smear         */
    float mix;          /* idx 9   [0 – 1]   dry/wet                          */
    float level;        /* idx 10  [0 – 2]   output gain                      */
